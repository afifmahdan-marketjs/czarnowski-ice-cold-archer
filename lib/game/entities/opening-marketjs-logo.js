		...
		EntityOpeningMarketJSLogo = ig.Entity.extend({
			...
			drawLogoShield: function (obj) {
				if (!this.logoCanvas) {
					...
					this.logoCanvas.width = Math.max(ig.system.width, ig.system.height) * 2;
					this.logoCanvas.height = Math.max(ig.system.width, ig.system.height) * 2;
				}
				...
				x = ig.system.width / 2 + obj.x - w / 2;
				y = ig.system.height / 2 + obj.y - h / 2;

				ctx = ig.system.context;
				ctx.globalAlpha = obj.alpha;
				ctx.drawImage(this.logoCanvas, 0, 0, w, h, x, y, w, h);
				ctx.globalAlpha = 1;
			...
			drawCover: function (obj) {
				...
				ctx.globalAlpha = obj.alpha;
				ctx.fillStyle = "#ffffff";
				ctx.fillRect(x - ig.system.width, 0, ig.system.width, ig.system.height);
				ctx.globalAlpha = 1;
				ctx.restore();
			},
		});
