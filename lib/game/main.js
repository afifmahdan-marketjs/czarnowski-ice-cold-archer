  ...
  .defines(function () {
    ...
    MyGame = ig.Game.extend({
      ...
      init: function () {
        ...
        /** Linkit360 Skill - mergeSettings */
        try {
          ig.linkit360Skill.mergeSettings({
            validator_id: "cool-archer-hd-linkit360-skill",
            project_id: "marketjs-linkit360-skill"
          });
        } catch(error) {}
      ...
      getLevelID: function (name) {
        for (var i = 0; i < this.director.levels.length; i++) {
          ...
          if (
            level.entities &&
            level.entities[0].type == "Entity" + name + "Controller"
          ) {
            return i;
          }
        }
      ...
      drawFPS: function () {
        ...
        this.fillStrokeText(
          "[ WEBGL " +
            ig.altBabylon.engine.webGLVersion +
            " ][ FPS C " +
            fps +
            " ][ FPS B " +
            ig.altBabylon.engine.performanceMonitor.averageFPS.toFixed() +
            " ]",
          5,
          953
        );
    ...
    ig.altBabylon
      .loadScene("media/scenes/", "game-scene.babylon", "#webglcanvas")
      .onSceneLoaded.addOnce(
        ig.babylonSceneController.init.bind(ig.babylonSceneController)
      );
    ig.main(
      "#canvas",
      MyGame,
