    ...
    .defines(function () {
        ...
        ig.Arrow = ig.MeshObject.extend({
            ...
            update: function (dt) {
                ...
                if (this.isFlying) {
                    ...
                    if (onTarget || onGround || onPost || onTrees) {
                        if (onTarget && !onGround && !onTrees && !onPost) this.onTarget = true;

                        ig.soundHandler.sfxPlayer.stop(ig.soundHandler.sfxPlayer.soundList.flight);
                        if (onTarget || onPost) {
                            ig.soundHandler.sfxPlayer.play(ig.soundHandler.sfxPlayer.soundList["arrowLand" + ig.random.int(1, 5)]);
                        }
                ...
                if (this.resetNextFrame) {
                    ...
                    if (camera.position.z <= -9.98 && Math.abs(ig.meshManager.targetHeight - camera.position.y) < 0.02) {
            ...
            launch: function () {
                ...
                ig.soundHandler.sfxPlayer.play(ig.soundHandler.sfxPlayer.soundList.flight);
            },
