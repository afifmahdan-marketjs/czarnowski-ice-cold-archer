ig.module("game.scene3d.objects.arrow")
  .requires("game.scene3d.objects.mesh-object")
  .defines(function () {
    "use strict";
    ig.Arrow = ig.MeshObject.extend({
      ...
      isHitTarget: function () {
        ...
        var arrow = this.mesh;
        if (this.mesh.intersectsMesh(ig.meshManager.target, false)) {
          var dx = arrow.position.x - target.position.x;
          var dy = arrow.position.y - target.position.y;
          var dSquared = dx * dx + dy * dy;
          var rSquared =
            ig.meshManager.targetRadius * ig.meshManager.targetRadius;
      ...
      calculateScore: function () {
        ...
        if (score < 0 || !this.onTarget) score = 0;
        this.lastScore = score;
        if (score == 0) {
          ig.soundHandler.sfxPlayer.play(
            ig.soundHandler.sfxPlayer.soundList.crowdMiss
          );
        } else if (score > 8) {
          ig.soundHandler.sfxPlayer.play(
            ig.soundHandler.sfxPlayer.soundList.crowdCheers
          );
      ...
      update: function (dt) {
        ...
        if (this.isFlying) {
          ...
          var gravityMultiplier =
            mesh.position.z > ig.meshManager.targetDistance ? 2 : 1;
          this.speed.x += (this.gravity.x + this.wind.x) * dt;
          this.speed.y +=
            (this.gravity.y + this.wind.y) * dt * gravityMultiplier;
          ...
          if (onTarget || onGround || onPost || onTrees) {
            if (onTarget && !onGround && !onTrees) this.onTarget = true;

            ig.soundHandler.sfxPlayer.stop(
              ig.soundHandler.sfxPlayer.soundList.flight
            );
            if (onTarget || onPost) {
              ig.soundHandler.sfxPlayer.play(
                ig.soundHandler.sfxPlayer.soundList[
                  "arrowLand" + ig.random.int(1, 5)
                ]
              );
            }
            ...
            setTimeout(
              function () {
                ...
                ig.meshManager.clearWindEffect();
          ...
          this.mesh.position.x +=
            (ig.cameraControl.camera.position.x + 1 - this.mesh.position.x) / 2;
          this.mesh.position.y +=
            (ig.cameraControl.camera.position.y - 2 - this.mesh.position.y) / 2;
          this.mesh.position.z +=
            (ig.cameraControl.camera.position.z + 10 - this.mesh.position.z) /
            2;
        }
        ...
        if (this.resetNextFrame) {
          ...
          var addedY = (ig.meshManager.targetHeight - camera.position.y) / 2;
          ...
          var addedZ = (-10 - camera.position.z) / 2;
          ...
          if (
            ...
            Math.abs(ig.meshManager.targetHeight - camera.position.y) < 0.02
          ) {
        ...
        if (ig.meshManager.bow.position.z > -bowDistance)
          ig.meshManager.bow.position.z = -bowDistance;
      },
      ...
      reset: function () {
        ...
        ig.cameraControl.targetRelativePosition.set(
          0,
          0,
          -ig.meshManager.targetDistance - 10
        );
        ...
        ig.meshManager.bow.isVisible = true;
      },
      ...
      setMainMenu: function () {
        ...
        ig.meshManager.bow.isVisible = false;

        if (!ig.babylonSceneController.postLines)
          ig.babylonSceneController.postLines = [];
