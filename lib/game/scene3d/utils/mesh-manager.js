ig.module("game.scene3d.utils.mesh-manager")
  .requires()
  .defines(function () {
    "use strict";
    ig.meshManager = {
      ...
      init: function () {
        ...
        setTimeout(
          function () {
            if (ig.altBabylon.scene.getMaterialByName("default material"))
              ig.altBabylon.scene.getMaterialByName(
                "default material"
      ...
      initGround: function () {
        ...
        this.groundMaterial.emissiveColor =
          BABYLON.Color3.FromHexString(groundColor);
        this.groundMaterial.ambientColor =
          BABYLON.Color3.FromHexString(groundColor);
        this.groundMaterial.diffuseColor =
          BABYLON.Color3.FromHexString(groundColor);
        ...
        this.stripeMaterial = new BABYLON.StandardMaterial(
          "stripeMaterial",
          scene
        );
        this.stripeMaterial.emissiveColor =
          BABYLON.Color3.FromHexString(stripeColor);
        this.stripeMaterial.ambientColor =
          BABYLON.Color3.FromHexString(stripeColor);
        this.stripeMaterial.diffuseColor =
          BABYLON.Color3.FromHexString(stripeColor);
        ...
        for (var i = 0; i < this.treeLineDistance + 300; i += stripeSize * 2) {
          var stripe = BABYLON.MeshBuilder.CreateBox(
            "stripe",
            { width: stripeWidth, height: 1, depth: stripeSize },
            scene
          );
      ...
      initTarget: function () {
        var scene = ig.altBabylon.scene;
        this.targetDynamicTexture = new BABYLON.DynamicTexture(
          "targetTexture",
          1024,
          scene,
          true
        ...
        this.targetMaterial = new BABYLON.StandardMaterial(
          "targetMaterial",
          scene
        );
        ...
        this.target = BABYLON.MeshBuilder.CreateCylinder(
          "target",
          {
            diameter: this.targetRadius * 2,
            height: 5,
            tessellation: 64,
            faceUV: targetFaceUV,
            faceColors: targetFaceColors,
          },
          scene
        );
      ...
      initArrow: function () {
        var scene = ig.altBabylon.scene;
        this.arrowTip = BABYLON.MeshBuilder.CreateSphere(
          "arrowTip",
          { diameter: 1 },
          scene
        );
        this.arrowTip.position.set(0, 20, 0);
        this.arrowTip.visibility = 0;
        ...
        this.aftershotCamPos = BABYLON.MeshBuilder.CreateSphere(
          "arrowTip",
          { diameter: 0.1 },
          scene
        );
        ...
        this.crosshairPoint = BABYLON.MeshBuilder.CreateSphere(
          "crosshairPoint",
          { diameter: 0.1 },
          scene
        );
      ...
      initWindIndicator: function () {
        ...
        this.screenCorner = BABYLON.MeshBuilder.CreateBox(
          "screenCorner",
          { width: 0.5, height: 0.5, depth: 0.5 },
          scene
        );
        this.screenCorner.setPivotMatrix(
          BABYLON.Matrix.Translation(0.5, 0.15, 1),
          false
        );

        var windPlane = BABYLON.MeshBuilder.CreateGround(
          "windPlane",
          { width: 2, height: 2 },
          scene
        );
        windPlane.material = new BABYLON.StandardMaterial(
          "WindIndicatorMat",
          scene
        );
        ...
        this.windIndicatorTexture = new BABYLON.DynamicTexture(
          "windPlaneTexture",
          512,
          scene,
          true
      ...
      initWindEffect: function () {
        ...
        for (var i = 0; i < this.windImages.length; i++) {
          var windPlane = BABYLON.Mesh.CreatePlane(
            "windPlane" + i,
            12,
            scene,
            false
          ...
          var windPlaneTexture = new BABYLON.DynamicTexture(
            "windTexture" + i,
            256,
            scene,
            true
      ...
      clearWindEffect: function () {
        while (this.winds.length > 0) {
          this.winds.pop().dispose();
        }
      },
      ...
      update: function (dt) {
        ...
        if (!this.windTextureHasDrawn) {
          ...
          for (var i = 0; i < this.windSources.length; i++) {
            ...
            if (image.loaded) {
              var windPlaneTexture =
                this.windSources[i].material.diffuseTexture;
      ...
      setPosition: function (
        ...
        var pOfst = fromLeft
          ? -1 + spacingWithBorder * 2
          : 1 - h * 2 - spacingWithBorder * 2;
